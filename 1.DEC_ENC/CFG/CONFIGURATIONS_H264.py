...
def GENERATE_COMMAND(cq, folder, video_path, codec_path, home_path, path_id, extra_param, width, height, subsample, bitdepth, num_frames, frames_per_unit, unit_size):
	...
	#comando completo
	#/ldecod.exe -d decoder.cfg -p InputFile=coded_{VIDEO}.h264 -p OutputFile={VIDEO}.yuv
	...
	#definindo o caminho que será salvo o arquivo de vídeo decodificado
	rawvideo_param = ' -p OutputFile=' + this_folder + '../decoded_video.yuv'
	
	#definindo o caminho do arquivo de log e o comando para forçar saída nesse arquivo
	output_filename = this_folder + 'log/dec_' + path_id +  ep_name + '.log'
