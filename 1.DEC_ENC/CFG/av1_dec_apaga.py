...
import CPU_LIST as CPU_LIST_PY
ALLOWED_CORES = CPU_LIST_PY.CPU_LIST

#Lista de CQs a serem utilizados, deixe descomentado o que tu preferir
import CQ_LIST as CQ_LIST_PY
CQ_LIST = CQ_LIST_PY.CQ_LIST 
...
def GENERATE_COMMAND(cq, folder, video_path, codec_path, home_path, path_id, extra_param, width, height, subsample, bitdepth, num_frames, frames_per_unit, unit_size):
	...
	output_filename = ' -o /dev/null'
	input_filename = ' ' + this_folder + 'video/coded_' + path_id +  ep_name + '.av1'
	csv_filename = this_folder + '../av1_' + str(cq) + '.csv'
	
	decode_command = codec_path + CODEC_NAME
	decode_command += output_filename
	decode_command += input_filename
	decode_command += ' > ' + csv_filename
	decode_command += ' &'
