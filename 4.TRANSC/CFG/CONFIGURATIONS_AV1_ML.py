...
def GENERATE_COMMAND(cq, folder, video_path, codec_path, home_path, path_id, extra_param, width, height, subsample, bitdepth, num_frames, frames_per_unit, unit_size, model_name):
	...
	this_folder = home_path + '/' + folder + '/cq_' + str(cq) + '/'
	...
	main_cfg_param  = ' --width=' + str(width) #width
	main_cfg_param += ' --height=' + str(height) #hight
	main_cfg_param += ' --fps=' + str(frames_per_unit) + '/' + str(unit_size) #fps
	...
	codfile_param = ' -o ' + this_folder + 'video/coded_' + path_id +  ep_name + model_name + '.av1'
	
	#definindo o caminho do vídeo de entrada
	rawvideo_param = ' ' + this_folder + '../decoded_video.y4m'
	
	#definindo o caminho do arquivo de log e o comando para forçar saída nesse arquivo
	output_filename = this_folder + 'log/out_' + path_id +  ep_name + model_name + '.log'
	logfile_param = ' > ' + output_filename + ' 2>&1'
	
	extra_param += ' --thesis_model_name=' + model_name
...
def DO_DOWNLOAD(codec_path):
	...
	files = glob.glob(focus_path + 'AV1/*')
	for i in range(len(files)):
		old_path = files[i]
		new_path = files[i].replace(focus_path + 'AV1/', '').replace('--', '/')
...
def DO_COMPILE(os_version, codec_path):
	...
	if os_version == 18.04:
		...
		cmake_command = 'cd ' + codec_path + ' && cmake -DCMAKE_C_FLAGS="-I/usr/include/python3.8 -lpython3.8 -lm" ..'
	elif os_version > 18.04:
		#Mas na maioria não, daí tem que compilar de forma genérica:
		cmake_command = 'cd ' + codec_path + ' && cmake -DCMAKE_C_FLAGS="-I/usr/include/python3.8 -lpython3.8 -lm" -DAOM_TARGET_CPU=generic ..'
	else:
		#Em caso de ubuntu mais velho, utilizar a seguinte chamada:
		cmake_command = 'cd ' + codec_path + ' && cmake -DCMAKE_C_FLAGS="-I/usr/include/python3.8 -lpython3.8 -lm" -DAOM_TARGET_CPU=generic -DENABLE_DOCS=0 ..'
