...
#Antes de selecionar o correto arquivo de configuração,
#capturar o decodificador utilizado
old_decoder = sys.argv[1]
	
#Selecionar o arquivo de configuração desejado
import CFG.CONFIGURATIONS_AV1_ML as CFG

sys.path.append('../3.ML/')
import LIST_OF_WIN_MODELS as list_of_models
...
import VIDEOS_PREDICAO as VIDEOS
...
class EXPERIMENT:
	...
	def export(self):
		...
		if (self.model_name == ''):
			text_to_show = str(self.codec_folder) + " under anchor"
		else:
			text_to_show = str(self.codec_folder) + " under " + str(self.model_name)
...
class CORE_CONTROL:
	...
	def print_queue(self):
		print("Nucleos livres: ", end='')
		copy_queue = queue.Queue()
		#capturando os valores e mostrando
		while not self.core_queue.empty():
			core = self.core_queue.get()
			copy_queue.put(core)
			print(core, end=',')
		
		#devolvendo os valores
		while not copy_queue.empty():
			self.core_queue.put(copy_queue.get())
		print("\n\n")
...
class EXPERIMENT_CONTROL:
	def __init__(self):
		...
		if(self.load_backup()):
			...
			list_of_ML = []
			for MLattributes in list_of_models.THIS_LIST:
				list_of_ML.append(list_of_models.name_model(MLattributes))
			
			#list_of_ML = [(str(alpha_value) + str(fit_value)) for alpha_value, fit_value in list_of_models.THIS_LIST]
				...
				for cq in CFG.CQ_LIST:
					for extra_p in extra_params:
						for codec_path in CFG.CODEC_PATHS:
							for model_name in list_of_ML:
								exp = EXPERIMENT(model_name,
								                 cq,
												 resolution,
												 video,
												 width,
...
if CFG.EXECUTE:
	...
	while ( fifo_control.execute_experiment() ):
		printlog("Aguarde. Já foram concluídos " + fifo_control.get_percentage_of_experiments_completed() + "% do experimento", end='\r')
		sleep(__WAITING_TIME__)
