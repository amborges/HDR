...
static void ALEX_THESIS(int frame_idx, int mi_col, int mi_row, BLOCK_SIZE bsize, int prediction_mode){
	...
	int partition_type;
	     if( bsize == BLOCK_4X4 ){   partition_type = PARTITION_NONE; }
	else if( bsize == BLOCK_4X8 ){   partition_type = PARTITION_VERT; }
	else if( bsize == BLOCK_8X4 ){   partition_type = PARTITION_HORZ; }
	else if( bsize == BLOCK_8X8 ){   partition_type = PARTITION_NONE; }
	else if( bsize == BLOCK_8X16 ){  partition_type = PARTITION_VERT; }
	else if( bsize == BLOCK_16X8 ){  partition_type = PARTITION_HORZ; }
	else if( bsize == BLOCK_16X16 ){ partition_type = PARTITION_NONE; }
	else if( bsize == BLOCK_16X32 ){ partition_type = PARTITION_VERT; }
	else if( bsize == BLOCK_32X16 ){ partition_type = PARTITION_HORZ; }
	else if( bsize == BLOCK_32X32 ){ partition_type = PARTITION_NONE; }
	else if( bsize == BLOCK_32X64 ){ partition_type = PARTITION_VERT; }
	else if( bsize == BLOCK_64X32 ){ partition_type = PARTITION_HORZ; }
	else if( bsize == BLOCK_64X64 ){ partition_type = PARTITION_NONE; }
	else{                            partition_type = PARTITION_SPLIT; }

	printf("%d,%d,%d,%d,%d,%d,%d\n", frame_idx, mi_col, mi_row, bsize, partition_type, prediction_mode, intra_inter);
}
...
static void decode_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  //ALEX_THESIS
  ALEX_THESIS(cm->current_video_frame, mi_col, mi_row, bsize, mi->mode);
}
