...
bool av1_rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  //Capturando a decisão do modelo
  int THESIS_ML_DECISION = THESIS_get_decision(bsize, mi_col, mi_row);
  
  /*
  //A princípio, o condicional abaixo força o libaom a aumentar a profundidade da árvore, caso necessário
  if(THESIS_ML_DECISION == THESIS_DO_SPLIT && 
     bsize <= x->sb_enc.max_partition_size && 
     bsize > 3){
    x->sb_enc.max_partition_size = bsize - 3;
  }
  */
...
BEGIN_PARTITION_SEARCH:
  ...
  if (x->must_find_valid_partition) {
    ...
    //Linha abaixo garante que o nosso modelo não vá funcionar caso houver problemas
    //em encontrar um rd-cost adequado. A linha ajuda a não entrar em loop infinito
    THESIS_ML_DECISION = THESIS_WORKS_NORMAL;
  }
  // Partition block source pixel variance.
  unsigned int pb_source_variance = UINT_MAX;
  
  //ALTERANDO AS VARIAVEIS IMPORTANTES
  if(THESIS_ML_DECISION == THESIS_DONT_SPLIT){
    //part_search_state.do_square_split               = false;
    //part_search_state.partition_none_allowed       &= true;
    //as tres de baixo precisam do &, pois se for é borda, elas não podem ser usadas
  ...
  else if(THESIS_ML_DECISION == THESIS_DO_SPLIT){
  ...
  *rd_cost = best_rdc;
  ...
  if (part_search_state.found_best_partition) {
    if (bsize == cm->seq_params->sb_size) {
      ...
      //THESIS SALVAR STATUS
      if (cm->show_frame || cm->showable_frame){
        //testar só o de cima e depois só o debaixo.
        //Só o debaixo tá pegando o inter, mas o intra só SB
        THESIS_get_superblock_tree(pc_tree, cm->current_frame.order_hint, mi_col, mi_row, TRUE);
