...
const arg_def_t *main_args[] = { &g_av1_codec_arg_defs.help,
                                 &g_av1_codec_arg_defs.THESIS_MODEL_NAME,
...
static int parse_stream_params(struct AvxEncoderConfig *global,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    } else if (arg_match(&arg, &g_av1_codec_arg_defs.THESIS_MODEL_NAME, argi)) {
    	config->THESIS_MODEL_NAME = arg.val;
...
#define SHOW(field) \
  ...
  //THESIS
  if(stream->config.THESIS_MODEL_NAME != NULL){
  	fprintf(stderr, "    THESIS_MODEL_NAME            = %s\n", stream->config.THESIS_MODEL_NAME);
  }
  else
  	fprintf(stderr, "    THESIS_MODEL_NAME            = (not used)\n");
...
static void encode_frame(struct stream_state *stream,
  ...
  /* THESIS
   * Passando a variável que capturei lá no inicio do programa
   * para a variável que estará acessível durante todo o laço
   * principal do programa, a qualquer momento que eu precisar
   */
  THESIS_set_global_values(stream->config.THESIS_MODEL_NAME, stream->config.out_fn, stream->config.cfg.g_w, stream->config.cfg.g_h);
